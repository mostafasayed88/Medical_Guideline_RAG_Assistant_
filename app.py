"""
ESC Evidence — Medical Guideline RAG Assistant (Streamlit, single file)

PDF extraction -> section-aware chunking -> hybrid (dense + BM25) retrieval
-> cross-encoder reranking -> grounded generation via OpenRouter.
...
Deploy on Streamlit Community Cloud:
    ...
    4. Upload PDFs from the sidebar once the app is running (or commit a
       data/ folder of PDFs alongside app.py).
"""
...
class RAGIndex:
    ...
    def __init__(self, documents: list[dict], embedder, reranker, config: RAGConfig):
...
if query:
    ...
    # Short chat history for conversational context (last few turns only)
    history = [
